...
// NOTE: The full EvmPrecompileStorageProvider integration tests below require a concrete
// EvmFactory to construct EvmInternals. They were originally written against Tempo's
// `TempoEvmFactory`. Porting them to Base's BaseEvmFactory is straightforward but requires
// a context setup (`Context::op()`, OpSpecId, etc.) that pulls in more deps than this
// keychain port needs. The keychain itself is exercised end-to-end via HashMapStorageProvider,
// so these tests are gated behind an opt-in feature for now.
#[cfg(all(test, feature = "evm-bridge-tests"))]
mod tests {
    use super::*;
    use alloy_primitives::{B256, address, b256, bytes, keccak256};
    ...
    use base_common_evm::BaseEvmFactory as TempoEvmFactory;
