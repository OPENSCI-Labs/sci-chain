...
// SCI patch — the test module below exercises `EvmPrecompileStorageProvider`
// against a real `TempoEvmFactory`, which depends on Tempo-internal crates
// (`tempo_evm`, `tempo_revm`) that SCI does not ship. We gate the whole
// module behind the `evm-bridge-tests` Cargo feature (off by default) so
// `cargo check -p sci-precompiles` stays clean. Keychain coverage runs via
