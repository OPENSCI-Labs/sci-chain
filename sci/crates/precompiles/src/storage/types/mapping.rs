...
mod tests {
    ...
    fn test_mapping_slot_matches_old_impl() {
        ...
        assert_eq!(u256.mapping_slot(slot), old_mapping_slot(u256.to_be_bytes::<32>(), slot),);
    }
    ...
    fn test_mapping_basic_properties() {
        ...
        assert_eq!(slot1.slot(), slot2.slot(), "same key should produce same slot");
        ...
        assert_ne!(slot_a.slot(), slot_b.slot(), "different keys should produce different slots");
    ...
    fn test_nested_mapping_basic_properties() {
        ...
        assert_eq!(final_slot.slot(), expected_final_slot, "final slot should use double-hash");

        // Property 3: Determinism - same keys always produce same slot
        let slot_a = &nested[key1][key2];
        let slot_b = &nested[key1][key2];
        assert_eq!(slot_a.slot(), slot_b.slot(), "same keys should produce same slot");
