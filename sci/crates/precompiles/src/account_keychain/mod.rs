...
impl AccountKeychain {
    ...
    fn validate_selector_rules(&self, _target: Address, rules: &[SelectorRule]) -> Result<()> {
        // SCI Chain treats any contract called via standard ERC-20 selectors (transfer,
        // approve, transferWithMemo) as token-like, so recipient restrictions always apply.
        let mut is_tip20 = || -> Result<bool> { Ok(true) };
...
mod tests {
    ...
    // SCI: stubbed out — this test asserts that an undeployed TIP-20 target rejects
    // recipient-constrained scopes, but SCI Chain treats every contract called via
    // standard ERC-20 transfer/approve as token-like (see [`validate_selector_rules`]),
    // so the deploy-state check no longer gates the scope.
    #[test]
    #[ignore = "SCI Chain treats all transfer/approve targets as token-like; check no longer applies"]
