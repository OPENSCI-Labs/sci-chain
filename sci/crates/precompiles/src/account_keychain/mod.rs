...
// `is_tip20` predicate in `validate_selector_rules` is therefore stubbed to
// return `true` unconditionally (see the closure below). Recipient
...
impl AccountKeychain {
    ...
    fn validate_selector_rules(&self, _target: Address, rules: &[SelectorRule]) -> Result<()> {
        // SCI patch — TIP-20 detection is stubbed to always return `true`. Upstream
        // Tempo gates `recipient`-constrained selector rules behind "is the target
        // actually a TIP-20?". SCI doesn't ship a TIP-20 factory (we use vanilla
        // ERC-20), so we accept recipient restrictions on *any* target whose
        // selector matches a constrained TIP-20 selector
        // (`transfer` / `approve` / `transferWithMemo`). This re-applies the SCI
        // divergence introduced during the v1.6 port — see CLAUDE.md Critical
        // Rule #5.
        let is_tip20 = || -> Result<bool> { Ok(true) };
...
mod tests {
    ...
    // SCI patch — Tempo upstream constructs default `KeyRestrictions` via
    // `tempo_alloy::provider::keychain::KeyRestrictions::default()`, which
    // round-trips through their `tempo_alloy` provider crate (not shipped in
    // SCI). The sol-typed `KeyRestrictions` here has a manually-supplied
    // `Default` shape — we build it inline.
    ...
    // SCI patch — ignored because SCI's `is_tip20` predicate is stubbed to
    // return `true` unconditionally (see CLAUDE.md Critical Rule #5 and the
    // `validate_selector_rules` patch in this file). The upstream assertion
    // "recipient-constrained scope rejected on undeployed TIP-20" no longer
    // holds in SCI — we treat every transfer/approve target as TIP-20-like.
    #[test]
    #[ignore = "SCI sets is_tip20→true so recipient-constrained scopes are always accepted"]
